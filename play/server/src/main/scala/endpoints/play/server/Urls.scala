...
trait Urls extends algebra.Urls {
  ...
  def malformedRequest(invalid: Invalid): Result =
    Results.BadRequest.sendEntity(HttpEntity.Strict(
      ByteString(s"[${invalid.errors.map(error => s""""${error.replaceAllLiterally("\"", "\\\"")}"""").mkString(",")}]"),
